...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  await db.execute(sql`
    ...
    BEGIN
      ...
      IF NOT EXISTS (SELECT FROM pg_tables WHERE schemaname = 'cms' AND tablename = 'projects') THEN
      ...
      -- Create projects_rels table if not exists
      IF NOT EXISTS (SELECT FROM pg_tables WHERE schemaname = 'cms' AND tablename = 'projects_rels') THEN
      ...
      -- Add column to locked documents rels if not exists
      IF NOT EXISTS (SELECT FROM information_schema.columns WHERE table_schema = 'cms' AND table_name = 'payload_locked_documents_rels' AND column_name = 'projects_id') THEN
